...
for i in range(len(TELI_STA)):
    ...
    if (
        DATA_STA.min() > 0
        and DATA_SIN_g.min() > 0
        and DATA_SIN_r.min() > 0
        and DATA_SIN_i.min() > 0
    ):
        fits_STA.writeto(
            "E:\\cGAN\\1k\\stack%d\\test\\stacked\\%s.fit"
            % (stacked_no, TELI_STA[i][-22:-4])
        )
        fits_SIN_g.writeto(
            "E:\\cGAN\\1k\\stack%d\\test\\single\\g\\%s.fit"
            % (stacked_no, TELI_SIN_g[i][-22:-4])
        )
        fits_SIN_r.writeto(
            "E:\\cGAN\\1k\\stack%d\\test\\single\\r\\%s.fit"
            % (stacked_no, TELI_SIN_r[i][-22:-4])
        )
        fits_SIN_i.writeto(
            "E:\\cGAN\\1k\\stack%d\\test\\single\\i\\%s.fit"
            % (stacked_no, TELI_SIN_i[i][-22:-4])
        )
...
for i in range(len(TRLI_STA)):
    ...
    if (
        ...
        fits_STA.writeto(
            "E:\\cGAN\\1k\\stack%d\\train\\stacked\\%s.fit"
            % (stacked_no, TRLI_STA[i][-22:-4])
        )
        fits_SIN_g.writeto(
            "E:\\cGAN\\1k\\stack%d\\train\\single\\g\\%s.fit"
            % (stacked_no, TRLI_SIN_g[i][-22:-4])
        )
        fits_SIN_r.writeto(
            "E:\\cGAN\\1k\\stack%d\\train\\single\\r\\%s.fit"
            % (stacked_no, TRLI_SIN_r[i][-22:-4])
        )
        fits_SIN_i.writeto(
            "E:\\cGAN\\1k\\stack%d\\train\\single\\i\\%s.fit"
            % (stacked_no, TRLI_SIN_i[i][-22:-4])
        )
...
for i in range(len(TELI_STA)):
    fits_col.writeto(
        "E:\\cGAN\\1k\\stack%d\\test\\single\\gi\\%s.fit"
        % (stacked_no, TELI_SIN_g[i][-22:-4])
    )
