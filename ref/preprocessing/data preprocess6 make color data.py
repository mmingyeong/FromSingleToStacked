...
IMG_STA = np.zeros(len(LIST_STA) * 260 * 260).reshape(len(LIST_STA), 260, 260)
IMG_SIN_g = np.zeros(len(LIST_STA) * 260 * 260).reshape(len(LIST_STA), 260, 260)
IMG_SIN_i = np.zeros(len(LIST_STA) * 260 * 260).reshape(len(LIST_STA), 260, 260)
for i in range(len(LIST_STA)):
...
TARY = np.asarray([-1, -2, 3, 4, 5, 6, 7, -1, -4])

# In[10]:


TARY = np.where(TARY < 0, 0.1, TARY)
...
IMG_SIN_g_p = np.where(IMG_SIN_g < 0, 0.01, IMG_SIN_g)
IMG_SIN_i_p = np.where(IMG_SIN_i < 0, 0.01, IMG_SIN_i)
...
# gicolor = (-2.5*(np.log(IMG_SIN_g*Fg+1000)/np.log(10))) - (-2.5*(np.log(IMG_SIN_i*Fi+1000)/np.log(10)))
gicolor = (-2.5 * (np.log(IMG_SIN_g_p + 10) / np.log(10)) - 24.437168) - (
    -2.5 * (np.log(IMG_SIN_i_p + 10) / np.log(10)) - 23.626959
)

# In[19]:


print(gicolor.min(), gicolor.max())
...
gicolor = (gicolor - mini) / (maxi - mini)

# In[10]:


ID_SIN_g[0][-27:-16] + "gi" + ID_SIN_g[0][-15:]
...
for j in SEP_5_test2:
    for i in range(len(ID_STA)):
        if str(ID_STA[i])[-15:-9] == str(ID_STA[j])[-15:-9]:
            TE_SIN_gi_ID.append(ID_SIN_g[i][-27:-16] + "gi" + ID_SIN_g[i][-15:])
            TE_SIN_gi_IMG.append(gicolor[i])

for j in SEP_5_tr2:
    for i in range(len(ID_STA)):
        if str(ID_STA[i])[-15:-9] == str(ID_STA[j])[-15:-9]:
            TR_STA_ID.append(ID_STA[i])
            TR_SIN_gi_ID.append(ID_SIN_g[i][-27:-16] + "gi" + ID_SIN_g[i][-15:])
            TR_SIN_gi_IMG.append(gicolor[i])
...
pix = 260
for i in range(len(TE_SIN_gi_ID)):
    fits_file = fits.PrimaryHDU(
        data=TE_SIN_gi_IMG[i].astype("float64").reshape(pix, pix)[2:258, 2:258]
    )

    fits_file.writeto(
        "E:\\cGan\\STNUM\\stack%d\\test\\single\\gi\\%s" % (stacked_no, TE_SIN_gi_ID[i])
    )

for i in range(len(TR_SIN_gi_ID)):
    fits_file = fits.PrimaryHDU(
        data=TR_SIN_gi_IMG[i].astype("float64").reshape(pix, pix)
    )

    fits_file.writeto(
        "E:\\cGan\\STNUM\\stack%d\\train\\single\\gi\\%s"
        % (stacked_no, TR_SIN_gi_ID[i])
    )
