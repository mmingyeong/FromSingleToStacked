...
for i in range(len(REF_LIST)):
    ...
    try:
        for REF_fields in pd.read_csv(REF_LIST[i], header=None).values[:,0]:
            MOVE_LIST.append(glob.glob('D:\\research\\stacked\\ALL\\%s\\%s\\*%d-%04d.fit.gz'%(REF_NS,band,REF_CC, REF_fields))) 
    except pandas.io.common.EmptyDataError:
        print('aaa')
    if len(MOVE_LIST) == 0 or len(MOVE_LIST[0]) == 0 :
        continue
    for j in range(len(MOVE_LIST)):
        shutil.move(MOVE_LIST[j][0], 'D:\\research\\stacked\\stack%d\\%s\\'%(stacked_no,band))
...
for i in range(len(REF_LIST)):
    ...
    if REF_NS == '206':
        ...
        for j in range(len(DOWN_LIST)):
            f.write('wget http://das.sdss.org/raw/200006/2/corr/%d/fpC-200006-r%d-%s\n' % (REF_CC, REF_CC, DOWN_LIST[j])) 
    elif REF_NS == '106':
        DOWN_LIST = []
        try:
            for REF_fields in pd.read_csv(REF_LIST[i], header=None).values[:,0]:
        ...
        if len(DOWN_LIST) == 0 or len(DOWN_LIST[0]) == 0 :
            continue
        for j in range(len(DOWN_LIST)):
            f.write('wget http://das.sdss.org/raw/100006/2/corr/%d/fpC-100006-r%d-%s\n' % (REF_CC, REF_CC, DOWN_LIST[j])) 
