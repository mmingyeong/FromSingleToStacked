...
(
    np.arcsinh(IMG_LOG((fits.open(FILE_LIST[0])[0].data - mini) / (maxi - mini)) * 10)
    / 3
).min()
...
plt.imshow(
    ASINH5((fits.open(FILE_LIST[Fno])[0].data - mini) / (maxi - mini)),
    cmap="Spectral_r",
    vmax=1.0,
    vmin=0.5,
)
...
plt.imshow(
    fits.open(FILE_LIST[Fno])[0].data, cmap="viridis", vmax=MEAN + STD, vmin=MEAN - STD
)
...
for Fno in range(len(FILE_LIST)):
    ...
    MEAN, MEDIAN, STD = sigma_clipped_stats(fits.open(FILE_LIST[Fno])[0].data)
    plt.imshow(
        fits.open(FILE_LIST[Fno])[0].data,
        cmap="gray_r",
        vmax=MEAN + STD,
        vmin=MEAN - STD,
    )
