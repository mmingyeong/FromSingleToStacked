...
if run == '206':
    ...
    single_run = '4203'
fg = open('D:\\research\\single\\stack%d\\g\\down_g%s_cc%d.sh' % (stacked_no, single_run, camcol),'w')
fr = open('D:\\research\\single\\stack%d\\r\\down_r%s_cc%d.sh' % (stacked_no, single_run, camcol),'w')
fi = open('D:\\research\\single\\stack%d\\i\\down_i%s_cc%d.sh' % (stacked_no, single_run, camcol),'w')
flist = open('D:\\research\\single\\stack%d\\flist_%s_%s_cc%d.csv'% (stacked_no, run, BAND, camcol),'w')
...
for stacked_no in [34]:
    for run in ['206']:
        ...
        for camcol in CAMCOLS:
            f_g = pd.read_csv('D:\\research\\single\\stack%d\\g\\down_g%s_cc%d.sh' % (stacked_no, single_run, camcol), sep='\s+',header=None)
...
stacked_no = 34
FILE_LIST_N = sorted(glob.glob('D:\\research\\stacked\\stack%d\\r\\*200006*.fit.gz'%(stacked_no)))
FILE_LIST_S = sorted(glob.glob('D:\\research\\stacked\\stack%d\\r\\*100006*.fit.gz'%(stacked_no)))
...
camcol = 1
print('(P.camcol = %d and P.run = %d and P.decmin > -400 and P.decmin < 400 and P.decmax > -400 and P.decmax < 400)'% (camcol, sorted(list(set(RUNS_for_count_N)))[0]))
for i in range(1,len(sorted(list(set(RUNS_for_count_N))))):
    print('  or (P.camcol = %d and P.run = %d and P.decmin > -400 and P.decmin < 400 and P.decmax > -400 and P.decmax < 400)'%(camcol, sorted(list(set(RUNS_for_count_N)))[i]))
...
print('%0.5f, %0.5f, %0.7f' % (stack34_N_cc1.values[:,-1].max(), stack34_N_cc1.values[:,-1].min(), stack34_N_cc1.values[:,-1].max() - stack34_N_cc1.values[:,-1].min()))
print('%0.5f, %0.5f, %0.7f\n' % (stack34_N_cc1.values[:,-2].max(), stack34_N_cc1.values[:,-2].min(), stack34_N_cc1.values[:,-2].max() - stack34_N_cc1.values[:,-2].min()))

print('%0.5f, %0.5f, %0.7f' % (stack34_N_cc2.values[:,-1].max(), stack34_N_cc2.values[:,-1].min(), stack34_N_cc2.values[:,-1].max() - stack34_N_cc2.values[:,-1].min()))
print('%0.5f, %0.5f, %0.7f\n' % (stack34_N_cc2.values[:,-2].max(), stack34_N_cc2.values[:,-2].min(), stack34_N_cc2.values[:,-2].max() - stack34_N_cc2.values[:,-2].min()))

print('%0.5f, %0.5f, %0.7f' % (stack34_N_cc3.values[:,-1].max(), stack34_N_cc3.values[:,-1].min(), stack34_N_cc3.values[:,-1].max() - stack34_N_cc3.values[:,-1].min()))
print('%0.5f, %0.5f, %0.7f\n' % (stack34_N_cc3.values[:,-2].max(), stack34_N_cc3.values[:,-2].min(), stack34_N_cc3.values[:,-2].max() - stack34_N_cc3.values[:,-2].min()))

print('%0.5f, %0.5f, %0.7f' % (stack34_N_cc4.values[:,-1].max(), stack34_N_cc4.values[:,-1].min(), stack34_N_cc4.values[:,-1].max() - stack34_N_cc4.values[:,-1].min()))
print('%0.5f, %0.5f, %0.7f\n' % (stack34_N_cc4.values[:,-2].max(), stack34_N_cc4.values[:,-2].min(), stack34_N_cc4.values[:,-2].max() - stack34_N_cc4.values[:,-2].min()))

print('%0.5f, %0.5f, %0.7f' % (stack34_N_cc5.values[:,-1].max(), stack34_N_cc5.values[:,-1].min(), stack34_N_cc5.values[:,-1].max() - stack34_N_cc5.values[:,-1].min()))
print('%0.5f, %0.5f, %0.7f\n' % (stack34_N_cc5.values[:,-2].max(), stack34_N_cc5.values[:,-2].min(), stack34_N_cc5.values[:,-2].max() - stack34_N_cc5.values[:,-2].min()))
