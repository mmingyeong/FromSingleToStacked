...
for BAND in bands:
    ...
    for i in range(len(LIST_STA)):
        IMG_STA[i] = fits.open(LIST_STA[i])[0].data[
            qpix : ISIZE - qpix, qpix : ISIZE - qpix
        ]
        IMG_SIN[i] = fits.open(LIST_SIN[i])[0].data[
            qpix : ISIZE - qpix, qpix : ISIZE - qpix
        ]
    ...
    for i in range(len(IMG_STA)):
        ...
        if (
            IMG_STA[i].min() / SIST_NormF[band_no, CC - 1] < -20
            or IMG_STA[i].max() / SIST_NormF[band_no, CC - 1] > 80000
        ):
            dellist_1.append(i)
...
for BAND in bands:
    ...
    for i in range(len(TR_STA_ID)):
        fits_file = fits.PrimaryHDU(
            data=TR_STA_IMG[i].astype("float64").reshape(pix, pix)
        )
        fits_file.writeto(SAVE_PATH + "train\\stacked\\%s\\%s" % (BAND, TR_STA_ID[i]))

    for i in range(len(TR_SIN_ID)):
        fits_file = fits.PrimaryHDU(
            data=TR_SIN_IMG[i].astype("float64").reshape(pix, pix)
        )
